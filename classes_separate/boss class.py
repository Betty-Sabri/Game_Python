...
for round_num in range(1, 21):
    ...
    orc_general.turn(round_num, [boss])
    if boss.hp <= 0:
        print(f"{boss.name} has been defeated! {orc_general.name} wins!")
        break

    boss.turn(round_num, [orc_general])
    if orc_general.hp <= 0:
        print(f"{orc_general.name} has been defeated! {boss.name} wins!")
        break